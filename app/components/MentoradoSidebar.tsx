...
import { Home, Route, PackageCheck, CheckSquare, ChevronLeft } from "lucide-react";

const nav = [
  { label: "Meu momento", href: "/mentorado", icon: Home },
  { label: "Minha trilha", href: "/mentorado/trilha", icon: Route },
  { label: "Entregáveis", href: "/mentorado/entregaveis", icon: PackageCheck },
  { label: "Próximos passos", href: "/mentorado/tarefas", icon: CheckSquare },
];
    ...
    <aside style={{ width: 240, minWidth: 240, background: "#F5F4F0", borderRight: "1px solid #E0DDD8", display: "flex", flexDirection: "column", height: "100vh", position: "sticky", top: 0 }}>
      <div style={{ padding: "24px 20px 20px", borderBottom: "1px solid #E0DDD8" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: "#1D9E75", marginTop: 1, fontWeight: 500 }}>Minha Jornada</div>
          </div>
        </div>
      </div>
      ...
      <nav style={{ flex: 1, padding: "12px 8px", overflowY: "auto" }}>
        {nav.map(({ label, href, icon: Icon }) => {
          const active = href === "/mentorado" ? pathname === href : pathname.startsWith(href);
          return (
            <Link
              key={href}
              href={href}
              style={{
                ...
                background: active ? "#1D9E7510" : "transparent",
                textDecoration: "none",
      ...
      <div style={{ padding: "12px 8px", borderTop: "1px solid #E0DDD8" }}>
        ...
        <div style={{ padding: "10px 12px", background: "white", borderRadius: 10, border: "1px solid #E0DDD8", display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#1D9E75", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontSize: 12, fontWeight: 600, flexShrink: 0 }}>AR</div>
          <div style={{ overflow: "hidden" }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#1A1A1A", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>Ana Rodrigues</div>
            <div style={{ fontSize: 11, color: "#999" }}>Mentorada DRUM</div>
          </div>
        </div>
      </div>
    </aside>
